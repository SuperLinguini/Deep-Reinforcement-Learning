...
class A3C:
    ...
    def _build_model(self):
        input_layer = Input(batch_shape=(None, NUM_STATE))
        dense_layer = Dense(16, activation='relu')(input_layer)

        out_actions = Dense(NUM_ACTIONS, activation='softmax')(dense_layer)
        out_value = Dense(1, activation='linear')(dense_layer)
...
class Agent:
    ...
    def act(self, s):
        ...
        if random.random() < eps:
            ...
            s = np.array([s])
            prediction = a3c.predict_p(s)[0]
            a = np.random.choice(NUM_ACTIONS, p=prediction)
